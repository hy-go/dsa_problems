...
The copy should consist of the same number of new nodes, where each new node has its value set to the value of its corresponding original node. 
Both the next and random pointer of the new nodes should point to new nodes in the copied list 
such that the original and copied list pointers represent the same list state. 
...
class Solution {
  public:
    Node *copyList(Node *head) {
        Node *temp = head, *nn, *next;
        while(temp) {
            next = temp->next;
            nn = new Node(temp->data);
            nn->next = next;
            temp->next = nn;
            temp = next;
        }
